...
def is_model_without_tool_support(model_name: str) -> bool:
    """Check if a model doesn't support tool calling."""
...
def clean_response_content(content: str) -> str:
    """Clean response content (remove reasoning tags, etc.)."""
    # Remove <think>...</think> tags for reasoning models
    think_pattern = re.compile(r'<think>.*?</think>', re.DOTALL)
    content = re.sub(think_pattern, '', content)
    
    # Remove stray closing tags
    stray_close_pattern = re.compile(r'.*?</think>', re.DOTALL)
    content = re.sub(stray_close_pattern, '', content)
